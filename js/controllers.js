...
$scope.alquiler={}
    ...
    $scope.limpiarClases = function(alquilable){
        alquilable=$scope.alquilables[alquilable-1];
        for (var i = 0; i < 20; i++) {
            alquilable.dias[i].hover = alquilable.dias[i].claseOriginal
        }
    }
    ...
    $scope.seleccionarBarril = function(barril){
        ...
        console.log (barril)
            ...
            if($scope.alquiler.desde){
                ...
                $scope.barrilSeleccionado.desdeDiaMysql = $scope.alquiler.desdeDiaIdParaMySql;
                $scope.barrilSeleccionado.hastaDiaMysql = $scope.alquiler.hastaDiaIdParaMySql;
        ...
        if ( $scope.resumen.alquilerTemp ){
            ...
            $scope.resumen.alquilerTemp.push(barrilTemp)
                ...
                function(inputValue){
                    ...
                    SweetAlert.swal({
                        title: "Ingresar Seña",
                        text: "¿Cuanto va a dejar de seña? Valor: $"+$scope.barrilSeleccionado.valor,
                        type: "input",
                        showCancelButton: false,
                    ...
                    function(valor){
                        ...
                        if (valor > $scope.barrilSeleccionado.valor) {
                            swal.showInputError("La seña no puede ser mayor al valor del alquiler.");
                            return false
                        }    
                        if (isNaN(valor)){
    ...
    alquilable=this.$parent.alquilables[alquilable-1];
        ...
        for (var i = 0; i < 21; i++) { //LIMPIO LA LINEA COMPLETA, EN CASO QUE HAYA CAMBIADO LA SELECCION

            if($scope.barrilSeleccionado && $scope.barrilSeleccionado.id == alquilable.id){

                if(alquilable.dias[i].id > $scope.barrilSeleccionado.desdeDiaId+1 && alquilable.dias[i].id <= $scope.barrilSeleccionado.hastaDiaId ){
               
                    //EN REALIDAD NO HAGO NADA ACA
            ...
            if(alquilable.dias[i].hover == hoy){
    ...
    var marcar = function(){
            ...
            desdeDiaIdParaMySql = alquilable.dias[dia-1].fechaMysql;
            hastaDiaIdParaMySql = alquilable.dias[dia].fechaMysql;
            ...
            $scope.alquiler.hastaDiaId=hasta;
            $scope.alquiler.desde=moment(desdeFecha).locale('es').format('dddd DD MMMM').toUpperCase();
            $scope.alquiler.desdeDiaId=desde;

            $scope.alquiler.desdeDiaIdParaMySql = desdeDiaIdParaMySql;
            $scope.alquiler.hastaDiaIdParaMySql = hastaDiaIdParaMySql;
    }
    ...
    alquilable.dias[dia-1].claseAnterior=alquilable.dias[dia-1].hover;
            ...
            if (alquilable.dias[dia-1].fecha.isBefore(moment().startOf('date').add(0,'days'))){
                ...
                $scope.alquiler.desdeDiaId="";
                $scope.alquiler.hasta="";
                $scope.alquiler.hastaDiaId="";
        ...
        //luigi
        $scope.alquilables = [];
        $http.get('http://blackhop.api.dessin.com.ar/lucascapo',{
            params : {
            ...
            for (var i = 0; i < $scope.resumen.alquilerTemp.length; i++) {
                ...
                for (var i = 0; i <  $scope.alquilables.length; i++) {
                    ...
                    for ( j = 0; j <  $scope.alquilables[i].dias.length; j++) {
                        ...
                        $scope.alquilables[i].dias[j].claseOriginal=disponible;
                        ...
                        if ($scope.alquilables[i].dias[j].estado=="vencido"){
                            ...
                            $scope.alquilables[i].dias[j].claseOriginal = vencido;
                        }
                        //
                        ...
                        if ($scope.alquilables[i].dias[j].estado == "alquilado"){
                            ...
                            $scope.alquilables[i].dias[j].claseOriginal = nope
                        }
...
function terminarVentaCtrl ($q,$http,$scope,$log,$uibModalInstance,SweetAlert,$uibModal,WizardHandler,clienteSeleccionado,resumen,flagDevAlq){
    ...
    $scope.imprimirAlquiler = function(alquiler) {
        ...
        qz.websocket.connect().then(function() {
            ...
            var printData = [{
                ...
                data: '<html>' +
                    ...
                    '        <p style="font-family: Impact, Charcoal, sans-serif;margin:0 11px; padding:0; font-size: 22px;font-weight: 600;">' +
                    $scope.alquiler.litros + ' ' + litroPlural +
                    '        </p>' +
                    '        <br style="line-height: 2px;">' +
                    '        <p style="font-family: Impact, Charcoal, sans-serif;margin:0 8px; padding:0;font-size: 12px;font-weight: 600;line-height: 0px;">' + $scope.alquiler.descripcion + '</p>' +
                    '        <br style="line-height: 2px;">' +
                    '        <p style="font-family: Tahoma, sans-serif;margin:0 11px; padding:0;font-size: 10px;font-weight: 600;">Fecha:</p><br style="line-height: 3px;">' +
                    '        <p style="font-family: Impact, Charcoal, sans-serif;margin:0 8px; padding:0;font-size: 12px;font-weight: 600;line-height: 0px;">' + $scope.alquiler.fecha + '</p>' +
                    '   </div>' +
                    '    <hr>' +
                    '</html>'
            }];
        ...
        console.log('SE CERRO imprimir alquiler');
    }
    ...
    $scope.imprimirReal = function (alquiler) {          
        console.log('caca')
    };

    $scope.imprimirASD = function (alquiler) {          
        var defered = $q.defer();
        ...
        SweetAlert.swal({
                title: "Cortar Cupon",
                text: "por favor cortá el papel en la impresora",
                showCancelButton: false,
                confirmButtonColor: "#DD6B55",
            ...
            function(isConfirm) {
                if (isConfirm ) {
                    $scope.imprimirReal();
                    defered.resolve(alquiler);
                    gilada= false;

                }
            });
        console.log('retorno promise')
        

    };

    $scope.recursion = function (alquileres,id) {   
    console.log('alquileres.length');       
    console.log(alquileres.length);       
        if(alquileres.length > id){

            $scope.imprimirASD(alquileres[id]).then(function(data){
                id++;
                console.log('sume 1 a id y es: '  + id);
                if(id <= alquileres.length){
                    $scope.recursion(alquileres,id);
    ...
    $scope.ok = function () {
        ...
        for (var i=0;i<$scope.resumen.productos.length;i++){
            ...
            if($scope.resumen.productos[i].productoReal.categoria == 'Alquiler'){
                ...
                itemImprimirAlquiler = $scope.resumen.productos[i].productoVirtual;
                ...
                item.idAlquiler = $scope.resumen.productos[i].productoVirtual.idAlquiler;
            }else{
            ...
            //WiP Armo el array de objetos "entregables" 
            //donde voy a listar las botellas, botellones, tapas y pintas
        ...
        if(true){
                    $http.post('http://blackhop.api.dessin.com.ar/api/pos/caja/venta', {
                        ...
                        if(cupon.codigo!="OK"){ //no genero cupon porque no es necesario (ie. vendí antares)
                            ...
                            //for por cada alquiler x 2 
                            // no mostrar modal, solo imprimir
                            console.log(response.cuponesAlquileres);
                            console.log($q);
                            
                            var arreglo = response.cuponesAlquileres;
                            $scope.recursion(arreglo,0);
                            //for (var i = 0; i < response.cuponesAlquileres.length; i++) {   
                                    /*
                            $scope.imprimirAlquileres()
                            arreglo.forEach(arreglo, function(data){
                                var subPromises = [fetchImg(data), fetchUser(data)];
                            ...
                            response.cuponesAlquileres.forEach(function(elem,array){
                                console.log("rANTES")
                                console.log(elem)
                                SweetAlert.swal({
                                    title: "Cortar Cupon",
                                ...
                                function(isConfirm) {
                                    if (isConfirm) {
                                        console.log("response.cuponesAlquileres[i]")
                                        console.log(elem)
                                    }
                                });
                                ...
                                */
                                /*
                                $scope.imprimirAlquiler(imprimirAlquileres[i]);
                                //hacer una pausa para el corte
                                $scope.imprimirAlquiler(imprimirAlquileres[i]);
                                */
                            //}
                           // });
...
function imprimirAlquilerCtrl($scope, $sanitize, $log, $uibModalInstance, alquiler) { //mariobross borrar

    /*
    Datos a Imprimir:

    Tipos de cerveza (limitar la cantidad de caracteres?)
    ...
    */
    ...
    $scope.imprimirAlquiler = function() {
        ...
        qz.websocket.connect().then(function() {
            ...
            var printData = [{
                ...
                data: '<html>' +
                    ...
                    '        </p>' +
                    '        <br style="line-height: 2px;">' +
                    '        <p style="font-family: Impact, Charcoal, sans-serif;margin:0 8px; padding:0;font-size: 12px;font-weight: 600;line-height: 0px;">' + $scope.alquiler.estilos + '</p>' +
                    '        <br style="line-height: 2px;">' +
                    '        <p style="font-family: Tahoma, sans-serif;margin:0 11px; padding:0;font-size: 10px;font-weight: 600;">Fecha:</p><br style="line-height: 3px;">' +
                    '        <p style="font-family: Impact, Charcoal, sans-serif;margin:0 8px; padding:0;font-size: 12px;font-weight: 600;line-height: 0px;">' + $scope.alquiler.fecha + '</p>' +
                    '   </div>' +
...
function imprimirCuponCtrl ($scope,$sanitize,$log,$uibModalInstance,cupon){
        ...
        var printData = [
        {
            ...
            +'        </div>'
            +'        <br style="line-height: 2px;">'
            +'        <p style="font-family: Impact, Charcoal, sans-serif; margin:0 14px; padding:0; font-size: 48px;font-weight: 600;">'
